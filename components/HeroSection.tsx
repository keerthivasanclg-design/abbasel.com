...
const ServicePill: React.FC<{ text: string; position: string }> = ({ text, position }) => (
  <div
    className={`hidden md:block absolute ${position} bg-gray-500 bg-opacity-50 backdrop-blur-sm text-white text-sm px-4 py-2 rounded-full shadow-lg`}
  >
    {text}
  </div>
);
...
const HeroSection: React.FC = () => {
  return (
    <section
      id="home"
      className="bg-black text-white min-h-screen flex items-center py-16 md:py-0"
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          <div className="flex flex-col space-y-6 text-left">
            <h1 className="text-2xl sm:text-3xl md:text-5xl font-extrabold leading-snug md:leading-tight">
              Maximize the Impact of Your Growth by Using the Best{" "}
              <span className="text-yellow-400">Strategies of Digital Marketing</span>{" "}
              and <span className="text-yellow-400">Fuel Your Business</span>{" "}
              with Digital Marketing Services
            </h1>

            <div className="flex items-center gap-2">
              <span className="bg-yellow-400 text-black font-bold text-base px-3 py-1">
                #
              </span>
              <span className="border border-white px-4 py-1 font-semibold text-sm">
                Karaikudi
              </span>
            </div>

            <p className="text-sm sm:text-base text-gray-300 max-w-lg">
              Foster the sustainable growth of your business with our expert
              digital marketing services. We use innovative marketing strategies
              that target high-quality leads while amplifying brand visibility
              to achieve unmatched ROI.
            </p>

            <button className="w-fit flex items-center gap-3 px-6 py-3 bg-yellow-400 text-black font-bold rounded-full hover:bg-yellow-500 transition-all duration-300 shadow-lg">
              <span>Book an Appointment</span>
              <div className="bg-white rounded-full p-1">
                <ArrowRightIcon className="w-4 h-4" />
              </div>
          ...
          <div className="relative flex justify-center items-center">
            <div className="w-[220px] h-[220px] sm:w-[300px] sm:h-[300px] md:w-[500px] md:h-[500px] rounded-full overflow-hidden border-8 border-gray-800 shadow-2xl">
              <img
                src={logo}
                alt="Abbasel Digital Marketing"
